...
GLOBAL void *um_malloc2(size_t size, char *file, int line )
{
	...
	if (buffer != NULL)	               /* Were we successfull?                */
	{
		...
		if (mptr != NULL)
		{
			char drive[3], path[512], filename[30], suffix[5];
			...
			/* Wir haengen uns vorne an die Liste an */
			mptr->next=anker;
			anker=mptr;
...
GLOBAL void um_free(void *memblock)
{
	...
	if ((tanker!=NULL)&&(memory_error==0))
	{
		do
		{
			...
			{
				/* Speicherfehler: unsere Checksumme wurde ueberschrieben! */
				lauf=0; /* Abbrechen, wenn die Zeiger ueberschrieben sind, kuennte es krachen */
				memory_error=42; /* Speicherfehler Flag setzen */
