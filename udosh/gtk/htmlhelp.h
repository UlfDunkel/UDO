...
#define HHWIN_PROP_AUTO_SYNC        (1l << 8)    /* automatically ssync contents and index */
...
#define HHWIN_TB_MARGIN             (1l << 28)   /* the window type has a margin */
...
#if (WINVER < 0x0400) && !defined(__MINGW32__)
...
{
    NMHDR   hdr;
    LPCSTR   pszUrl; /* Multi-byte, null-terminated string */
} HHN_NOTIFY;
...
{
    NMHDR   hdr;
    LPCSTR   pszCurUrl;      /* Multi-byte, null-terminated string */
...
GtkWidget *HtmlHelp(GtkWidget *hwndCaller, const char *pszFile, unsigned int uCommand, void *dwData);
