...
kwset_t kwsalloc(char const *trans)
{
	...
	if (kwset != NULL)
	{
		...
		kwset->target = 0;
		kwset->trans = trans;
		kwset->trie = (struct trie *) malloc(sizeof(struct trie));
...
int kwsincr(kwset_t kws, char const *text, size_t len)
{
	struct kwset *kwset;
	...
	while (len--)
	{
		...
		if (!link)
		{
			link = (struct tree *) malloc(sizeof(struct tree));

			if (!link)
				return FALSE;
			link->llink = 0;
			link->rlink = 0;
			link->trie = (struct trie *) malloc(sizeof(struct trie));
...
int kwsprep(kwset_t kws)
{
	...
	if (kwset->words == 1 && kwset->trans == 0)
	{
		...
		kwset->target = malloc(kwset->mind);
...
static void kwsfreetrie(struct trie *trie)
{
	struct trie *next;
